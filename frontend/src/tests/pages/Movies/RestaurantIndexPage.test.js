...
describe("MovieIndexPage tests", () => {
    ...
    test("renders without crashing for regular user", () => {
        ...
        axiosMock.onGet("/api/movies/all").reply(200, []);
    ...
    test("renders three movies without crashing for regular user", async () => {
        setupUserOnly();
        const queryClient = new QueryClient();
        axiosMock
            .onGet("/api/movies/all")
            .reply(200, movieFixtures.threeMovies);
    ...
    test("renders three movies without crashing for admin user", async () => {
        setupAdminUser();
        const queryClient = new QueryClient();
        axiosMock
            .onGet("/api/movies/all")
    ...
    test("renders empty table when backend unavailable, user only", async () => {
        ...
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/movies/all").timeout();
        ...
        expect(errorMessage).toMatch(
            "Error communicating with backend via GET on /api/movies/all"
        );
        restoreConsole();
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock
            .onDelete("/api/movies")
            .reply(200, "Movie with id 2 was deleted");
        ...
        await waitFor(() => {
            expect(mockToast).toBeCalledWith("Movie with id 2 was deleted");
        });
    });
});
